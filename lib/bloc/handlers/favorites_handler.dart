...
mixin FavoritesHandler on Bloc<AppEvent, AppState> {
  ...
  Future<void> onLoadFavorites(LoadFavorites event, Emitter<AppState> emit) async {
    ...
    try {
      final List<FavoriteItem> favoriteItems = await _favoritesService.getFavorites();
      
      final List<int> movieIds = favoriteItems
          .where((item) => item.mediaType == MediaType.movies.toJsonField())
          .map<int>((item) => item.mediaId)
          .toList();
      final List<int> tvShowIds = favoriteItems
  ...
  Future<void> _addFavorite(dynamic media, MediaType mediaType) async {
    try {
      ...
      if (mediaType == MediaType.movies) {
        await _favoritesService.addMovie(id, media.title, media.posterPath);
      } else if (mediaType == MediaType.tvShows) {
        await _favoritesService.addTvShow(id, media.name, media.posterPath);
      }
    } catch (e, s) {
