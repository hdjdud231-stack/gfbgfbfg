...
class EmbedSuExtractor implements BaseStreamExtractor {
  EmbedSuExtractor() {
  ...
  final String _providerKey = "embedSu";
  final String _baseUrl = "https://embed.su/embed";
  ...
  String _buildStreamUrl(StreamExtractorOptions options) {
    ...
    if (options.season != null && options.episode != null) {
      // TV Show format: https://embed.su/embed/tv/{tmdb_id}/{season}/{episode}
  ...
  Future<MediaStream?> getStream(StreamExtractorOptions options) async {
    try {
      _logger.i("Attempting to get stream from EmbedSu for TMDB ID: ${options.tmdbId}");
      
      String streamUrl = _buildStreamUrl(options);
      _logger.i("EmbedSu stream URL: $streamUrl");
      ...
      _logger.e("Error in EmbedSuExtractor", error: e, stackTrace: s);
    }
