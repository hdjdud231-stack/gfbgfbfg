...
class _SemoPlayerState extends State<SemoPlayer> with TickerProviderStateMixin {
  ...
  Future<void> _initializePlayer() async {
    try {
      ...
      // Initialize video player with better buffering
      await _videoPlayerController.initialize().catchError((Object? e) {
        widget.onError?.call(e);
        return;
      });
      
      // Set video player options for better performance
      await _videoPlayerController.setVolume(1.0);
      
      // Enable looping to prevent playback issues
      ...
      if (mounted) {
        ...
        // Start progress update timer with better frequency
  ...
  Future<void> _playerListener() async {
    try {
      ...
      if (isPlaying && isBuffering) {
        // Allow some time for buffering before showing indicator
  ...
  Future<void> _loadArabicSubtitles() async {
    try {
      // Show loading indicator
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('جاري تحميل الترجمة العربية...'),
      ...
      if (widget.tmdbId == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('معرف TMDB غير متوفر'),
      ...
      // Try to get Arabic subtitles
      final List<File> arabicSubtitles = await subtitleService.getSubtitles(
        widget.tmdbId!,
        seasonNumber: widget.seasonId,
        episodeNumber: widget.episodeId,
        locale: "AR", // Arabic locale
      );

      if (arabicSubtitles.isNotEmpty) {
        // Load the first Arabic subtitle file
        _subtitleController.updateSubtitleContent(
          content: await arabicSubtitles.first.readAsString(),
        );
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('تم تحميل الترجمة العربية بنجاح!'),
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('لم يتم العثور على ترجمة عربية لهذا المحتوى'),
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('خطأ في تحميل الترجمة: ${e.toString()}'),
          backgroundColor: Colors.red,
                ...
                child: AppBar(
                  ...
                  actions: <Widget>[
                    ...
                    IconButton(
                      ...
                      onPressed: () => _loadArabicSubtitles(),
                      tooltip: 'ترجمة عربية',
                    ),
